        ...
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            <div class="flex flex-col justify-center gap-2">
                ...
                <div class="flex items-center gap-2.5">
                    <a class="btn text-center btn-sm btn-primary" href="{{ route('workforce.submitted-form.leave.index') }}">
                        <i class="ki-filled ki-left"></i></i>Kembali
                    </a>
                    @if ($data['leave']['status']->value === 2)
                        <form action="{{ route('workforce.submitted-form.leave.confirm-leave-update',$data['leave']['id']) }}" method="POST" onsubmit="return confirmLeaveApproval(event)"
                            class="inline">
                            @csrf
                            <input type="hidden" name="_method" value="PUT">
                            <button type="submit" class="btn text-center btn-sm btn-success" data-tooltip="#confirmLeave">
                                <i class="ki-filled ki-check"></i> Setujui Cuti
                            </button>
                        </form>

                        <form action="{{ route('workforce.submitted-form.leave.reject-leave-update',$data['leave']['id']) }}" method="POST" onsubmit="return rejectLeave(event)" class="inline">
                            @csrf
                            <input type="hidden" name="_method" value="PUT">
                            <button type="submit" class="btn text-center btn-sm btn-danger" data-tooltip="#rejectLeave">
                                <i class="ki-filled ki-cross"></i> Tolak Cuti
                            </button>
                        </form>
                    @endif
            </div>
                ...
                <div class="card-body grid gap-5">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        ...
                        <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5">
                            ...
                            <input class="input" name="start_date" placeholder="mm/dd/yyyy" type="date" readonly="true"
                                value="{{ $data['leave']['start_date'] }}" />
                        </div>
                        <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5">
                            <label class="form-label max-w-56">
                                Sampai Tanggal
                            </label>
                            <input class="input" name="end_date" placeholder="mm/dd/yyyy" type="date" readonly="true"
                                value="{{ $data['leave']['end_date'] }}" />
                        </div>
                    </div>
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
