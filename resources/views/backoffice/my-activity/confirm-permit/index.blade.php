...
@push('javascript')
    ...
    <script type="text/javascript">
        const showRoute = "{{ route('my-activity.my-permit-confirm.confirm-permit-show', ':id') }}"; // Pass route pattern to JS
        const editRoute = "{{ route('my-activity.my-permit-confirm.confirm-permit-update', ':id') }}"; // Pass route pattern to JS
        const rejectRoute = "{{ route('my-activity.my-permit-confirm.reject-permit-update', ':id') }}"; // Pass route pattern to JS
    ...
    <script type="text/javascript">
        const apiUrl = '{{ route('api.v1.my-activity.my-permit-confirm.datatable-confirm-permit') }}';
        const deleteUrl = '#';
            ...
            columns: {
                ...
                action: {
                    ...
                    render: (data, type, row) => {
                        ...
                        const buttonApprove = `
                            <form action="${editUrl}" method="POST" onsubmit="return confirmPermitApproval(event)" class="inline">
                                @csrf
                        ...
                        const buttonReject = `
                            <form action="${rejectUrl}" method="POST" onsubmit="return rejectPermit(event)" class="inline">
                                @csrf
                                <input type="hidden" name="_method" value="PUT">
                                <button type="submit" class="btn btn-icon btn-sm btn-clear btn-danger" data-tooltip="#rejectPermit">
                                    <i class="ki-filled ki-cross"></i>
                                </button>
                            </form>
                            <div class="tooltip transition-opacity duration-300" id="rejectPermit">
