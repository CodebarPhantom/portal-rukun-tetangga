...
@section('content')
    ...
    <!-- Container -->
    <form action="{{ route('workforce.employee-loan.update',$data['loan']['id']) }}" method="post">
        @csrf
        @method('PUT')
                    ...
                    <div class="card-body grid gap-5">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5">
                                ...
                                <input value="{{ old('loan_amount',$data['loan']['formatted_loan_amount']) }}" type="text" placeholder="10,000,000"
                                    name="loan_amount" required class="input w-full border border-gray-300 p-2 text-right format-number">
                            </div>
                            <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5">
                                <label class="form-label max-w-56">
                                    Periode Angsuran
                                </label>
                                <input value="{{ old('installment_period',$data['loan']['installment_period']) }}" type="number" min="1" placeholder="3"
                                    name="installment_period" required class="input w-full border border-gray-300 p-2 text-right">
                            </div>
                        </div>
                        <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5">
                            <label class="form-label max-w-56">
                            ...
                            <textarea class="textarea" name="notes" placeholder="Catatan pinjaman" rows="3" type="text" />{{ $data['loan']['notes'] }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
