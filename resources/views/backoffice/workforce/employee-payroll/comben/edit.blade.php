...
@section('content')
    <!-- Container -->
    <form action="{{ route('workforce.employee-payroll-comben.update', $data['comben']['id']) }}" method="post">
        @csrf
        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5">
                                ...
                                <input class="input" name="note" placeholder="Catatan Compensation" type="input"
                                    value="{{ old('note', $data['comben']['note']) }}" />
                            </div>
                            <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5">
                                <label class="form-label max-w-56">
                                    Nominal
                                </label>
                                <input value="{{ old('value', number_format($data['comben']['value'], 2, '.', ',')) }}"
