        ...
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            ...
            <div class="flex items-center gap-2.5">
                {{-- <div class="input-group">
                    <input id="start_date" class="input" placeholder="Tanggal Mulai Lembur" type="datetime-local" value="" />

                    <input id="end_date" class="input" placeholder="Tanggal Akhir Lembur" type="datetime-local" value="" />
                </div> --}}
                <button id="refresh-btn" class="btn btn-sm text-center btn-info">
                    <i class="ki-filled ki-arrows-circle"></i>
                </button>
                <a class="btn btn-sm text-center btn-success" href="{{ route('my-activity.my-overtime.create') }}">
...
@push('javascript')
    ...
    <script type="text/javascript">
        const showRoute = "{{ route('my-activity.my-overtime.show', ':id') }}"; // Pass route pattern to JS
        const editRoute =
        "{{ route('my-activity.my-overtime.cancel-overtime-update', ':id') }}"; // Pass route pattern to JS
    </script>
    <!-- End - Define Route -->

    <script type="text/javascript">
        function confirmOvertimeCancel(event) {
            if (!confirm("Are you sure you want to cancel this overtime?")) {
    ...
    <script type="text/javascript">
        const apiUrl = '{{ route('api.v1.my-activity.my-overtime.datatable') }}';
        const deleteUrl = '#';
            ...
            columns: {
                ...
                action: {
                    ...
                    render: (data, type, row) => {
                        ...
                        const buttonCancel = `<form action="${editUrl}" method="POST" onsubmit="return confirmOvertimeCancel(event)" class="inline">
                                @csrf
                                <input type="hidden" name="_method" value="PUT">
                                <button type="submit" class="btn btn-icon btn-sm btn-clear btn-danger" data-tooltip="#confirmOvertimeCancel${type.id}">
                                    <i class="ki-filled ki-cross"></i>
                                </button>
                            </form>
                            <div class="tooltip transition-opacity duration-300" id="confirmOvertimeCancel${type.id}">
