    ...
    <div class="container-fixed">
        <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
            ...
            <div class="flex items-center gap-2.5">
                ...
                <a class="btn btn-sm text-center btn-success" href="{{ route('workforce.submitted-form.leave.create') }}">
...
@push('javascript')
    ...
    <script type="text/javascript">
        const confirmRoute = "{{ route('workforce.submitted-form.leave.confirm-leave-update', ':id') }}";
        const rejectRoute = "{{ route('workforce.submitted-form.leave.reject-leave-update', ':id') }}";
        const editRoute = "{{ route('workforce.submitted-form.leave.edit', ':id') }}";
        const showRoute = "{{ route('workforce.submitted-form.leave.show', ':id') }}";
    ...
    <script type="text/javascript">
        const apiUrl = '{{ route('api.v1.workforce.submitted-form.leave.dataTable') }}';
        const deleteUrl = "#";
            ...
            columns: {
                ...
                action: {
                    ...
                    render: (data, type, row) => {
                        ...
                        const buttonApprove = `
                            <form action="${confirmUrl}" method="POST" onsubmit="return confirmLeaveApproval(event)" class="inline">
                                @csrf
                                <input type="hidden" name="_method" value="PUT">
                                <button type="submit" class="btn btn-icon btn-sm btn-clear btn-success" data-tooltip="#confirmLeave">
                                    <i class="ki-filled ki-check"></i>
                                </button>
                            </form>
                            <div class="tooltip transition-opacity duration-300" id="confirmLeave">
                                Setujui {{ $data['pageTitle'] }}
                            </div>
                        `;

                        const buttonReject = `
                            <form action="${rejectUrl}" method="POST" onsubmit="return rejectLeave(event)" class="inline">
                                @csrf
                                <input type="hidden" name="_method" value="PUT">
                                <button type="submit" class="btn btn-icon btn-sm btn-clear btn-danger" data-tooltip="#rejectLeave">
                                    <i class="ki-filled ki-cross"></i>
                                </button>
                            </form>
                            <div class="tooltip transition-opacity duration-300" id="rejectLeave">
                        ...
                        <div class="items-center space-x-2">
                            ${buttonShow}  ${type.status ===  2 ? buttonEdit+buttonApprove+buttonReject : ''}
                        </div>
                    `;
                    },
                    createdCell(cell) {
